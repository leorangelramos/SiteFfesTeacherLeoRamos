...
- [cite_start]**Navegação por Abas e Acordeões:** Interface organizada que permite alternar entre _Exercícios, Listening, Vocabulário, Speaking, Writing e Reading_ sem recarregar a página.
- **Gamificação (Sistema de XP):** Os alunos ganham pontos de experiência (XP) ao completar exercícios corretamente. [cite_start]O progresso é salvo localmente no navegador (`localStorage`).
- [cite_start]**Reconhecimento de Voz (Speaking):** Utiliza a **Web Speech API** para ouvir a pronúncia do aluno e comparar com a frase alvo, fornecendo feedback imediato (Sucesso/Erro).
- [cite_start]**Busca Inteligente:** Barra de pesquisa com auto-complete que indexa todos os links de exercícios e opções do site, facilitando a navegação.
- **Exercícios Interativos:**
  - [cite_start]_Writing:_ Validação de respostas escritas com feedback de erro específico.
  - [cite_start]_Reading/Ordenação:_ Funcionalidade de "arrastar e soltar" (botões de subir/descer) para reorganizar parágrafos de textos.
- [cite_start]**Design Responsivo:** Layout adaptável para mobile e desktop, com menu fixo e identidade visual consistente[cite: 2, 3].

## 🛠️ Tecnologias Utilizadas

O projeto foi construído com tecnologias web padrão (Vanilla), garantindo alta performance e compatibilidade.

- [cite_start]**HTML5:** Estrutura semântica e acessibilidade (ARIA attributes)[cite: 2].
- [cite_start]**CSS3:** Variáveis CSS (`:root`), Flexbox, CSS Grid e design responsivo via Media Queries[cite: 3].
- [cite_start]**JavaScript (ES6+):** Lógica de interação, manipulação do DOM e APIs do navegador.
- [cite_start]**SEO & Metadados:** Configuração completa de Open Graph (OG Tags), Twitter Cards e JSON-LD para Rich Snippets (Schema.org)[cite: 2].
...
Certifique-se de que a estrutura de pastas (CSS, JS, Images) esteja mantida.
